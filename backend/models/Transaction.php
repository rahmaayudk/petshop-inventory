...
class Transaction {
    ...
    public function create() {
        ...
        try {
            $query = "INSERT INTO " . $this->table_name . " 
                     SET product_id=:product_id, user_id=:user_id, type=:type, qty=:qty, date=:date, notes=:notes";
    ...
    public function getBestSellersByDateRange($start_date, $end_date, $limit = 5) {
        $query = "SELECT p.id, p.name, p.sku_code, p.price_sell, p.price_buy, SUM(t.qty) as total_sold 
                  FROM " . $this->table_name . " t 
                  JOIN products p ON t.product_id = p.id 
                  WHERE t.type = 'out' AND t.date BETWEEN :start_date AND :end_date
                  GROUP BY p.id, p.name, p.sku_code, p.price_sell, p.price_buy
                  ORDER BY total_sold DESC 
                  LIMIT :limit";
    ...
    public function getSalesData($start_date, $end_date) {
        // Logika stok:
        // Stok Awal = current_stock - (stok dalam periode) = stok yang ada sebelum periode mulai
        // Stok Akhir = current_stock (stok actual saat ini)
        // Atau bisa dihitung: Stok Awal + Barang Masuk - Barang Keluar = Stok Akhir
    ...
    public function getRecentTransactions($limit = 10) {
        $query = "SELECT t.*, p.name as product_name, p.sku_code, u.username as performed_by, u.role as performed_role 
                  FROM " . $this->table_name . " t 
                  JOIN products p ON t.product_id = p.id 
