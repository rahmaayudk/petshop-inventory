...
export const transactionAPI = {
    ...
    getBestSellers: (limit = 5, start_date = null, end_date = null) => {
        let url = `/transactions?action=best_sellers&limit=${limit}`;
    ...
    getRecentTransactions: (limit = 10, detailed = false) => {
        const detailParam = detailed ? '&detail=1' : '';
        return api.get(`/transactions?action=recent&limit=${limit}${detailParam}`)
            .then(ensureArray)
            .catch(error => {
...
export const userAPI = {
    getAll: () => api.get('/users').then(response => Array.isArray(response) ? response : []),
    create: (userData) => api.post('/users', userData),
    update: (userData) => api.put('/users', userData),
    delete: (id) => api.delete('/users', { data: { id } })
};
